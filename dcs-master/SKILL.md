...
> **版本检查**：每次使用本 skill 前，先运行版本检查。如检测到新版本，在回复开头提示用户。
> 检查命令：`python3 ~/.qoderwork/skills/dcs-master/scripts/version-check-wrapper.py`

本 skill 用于在协同开发平台（DCS）使用场景下提供统一的流程指导、故障诊断、代码修复和浏览器协作能力。

## 使用前的版本检查

每次加载本 skill 时，应首先执行以下操作：

1. 运行版本检查脚本获取更新状态
2. 如果有新版本，在回复开头显示更新提示
3. 然后继续处理用户请求

版本检查脚本位置：`scripts/version-check-wrapper.py`
